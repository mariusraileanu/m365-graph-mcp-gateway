...
if command -v docker >/dev/null 2>&1 && container_running "${CONTAINER_NAME}"; then
  ...
  cat "${HOST_CLIPPY_DIR}/config.json" | docker exec -i "${CONTAINER_NAME}" sh -lc "umask 077; cat > '${CONTAINER_CLIPPY_DIR}/config.json'"
  cat "${HOST_CLIPPY_DIR}/token-cache.json" | docker exec -i "${CONTAINER_NAME}" sh -lc "umask 077; cat > '${CONTAINER_CLIPPY_DIR}/token-cache.json'"
  if [[ -f "${HOST_CLIPPY_DIR}/storage-state.json" ]]; then
    cat "${HOST_CLIPPY_DIR}/storage-state.json" | docker exec -i "${CONTAINER_NAME}" sh -lc "umask 077; cat > '${CONTAINER_CLIPPY_DIR}/storage-state.json'"
  fi
