...
TELEGRAM_TARGET="${OPENCLAW_TELEGRAM_TARGET_ID:-unset}"
mkdir -p "$CRON_WS_DIR"

cat > "${CRON_WS_DIR}/AGENTS.md" <<EOF
# Cron Agent (${OWNER_NAME})

- Owner: ${OWNER_NAME}
- Timezone: ${LOCAL_TZ}
- Primary channel: Telegram (\`${TELEGRAM_TARGET}\`)
...
- \`/home/node/.openclaw/workspace-cron/bin/oc_weather_local_json\`
