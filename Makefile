...
azure-add:
	@[ -n "$(USER)" ] || (echo "Usage: make azure-add USER=jdoe" && exit 1)
	bash scripts/azure.sh add $(USER)

azure-remove:
	@[ -n "$(USER)" ] || (echo "Usage: make azure-remove USER=jdoe" && exit 1)
	bash scripts/azure.sh remove $(USER)

azure-login:
	@[ -n "$(USER)" ] || (echo "Usage: make azure-login USER=jdoe" && exit 1)
...
azure-logs:
	@[ -n "$(USER)" ] || (echo "Usage: make azure-logs USER=jdoe" && exit 1)
